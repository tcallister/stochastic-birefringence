...
results = compute_likelihood_grids(zs_merger,dRdV_delayed,clippingFunction=toClip,kappaGridSize=400,baselines=['HLO2'])

# Create hdf5 file and write posterior samples
hfile = h5py.File('./../data/fixed_rate_delayedSFR_HLO2.hdf','w')
posterior = hfile.create_group('result')
for key,val in results.items():
    posterior.create_dataset(key,data=val)

# Add some metadata
hfile.attrs['Created_by'] = "run_birefringence_delayedSFR_HLO2.py"
